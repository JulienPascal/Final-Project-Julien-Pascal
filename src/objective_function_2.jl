...
###############################################
# Version of the objective function to be used
# by BlackBoxOptim and the simulated annealing 
# algorithm
##############################################
function objective_function_2(parameters, Weighting_matrix::Matrix, random_numbers::Vector, b0)
    ##################
...
#Moments to match:
mean_prod = mean(y_r[discard:(number_periods-1),1]) # Productivity yt
std_prod = std(log(y_r[discard:(number_periods-1),1]))

mean_unemployment = mean(ut_r[discard:(number_periods-1),1]) #Unemployment ut
std_unemployment = std(log(ut_r[discard:(number_periods-1),1]))

mean_exit_rate = mean(ft_r[discard:(number_periods-1),1]) #Exit rate from unemployment ft
...
#b0 = [ 1; 0.023;  0.059;  0.210; 2.374; 0.761] #with my empirical momemnts

#b1 = [mean_prod; std_prod; mean_unemployment; std_unemployment; mean_exit_rate; std_exit_rate ]; #simulated moments
#b0 = [ 1; 0.023;  0.059;  0.218; 0.762; 0.2358] #without kurtosis, with the standard deviation of the exit rate
...
# mean productivity, std productivity, mean unemployment rate, std unemployment rate, mean job finding rate, std job finding rate, mean job separation rate
...
#params_opt = [0.850196, 0.897381, 0.738404, 0.890517, 0.892101,  0.822158, 0.738211 ]

b0 = [1.0; 0.023;  0.059;  0.210; 0.761; 0.085; 0.046; 0.156]

params_opt = [0.77, 0.023, 0.99, 2.00, 5.56, 0.73, 0.64]
Weighting_matrix = eye(8,8)./(b0.*b0)

objective_function_2(params_opt, Weighting_matrix,random_numbers,b0)
"""
